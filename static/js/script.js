...
(function () {
  ...
  // Handle the action when a button is clicked for input fields
function buttonAction(action) {
    const inputElement = document.getElementById(action);
    const value = inputElement.value;
    
    if (value) {
        console.log(`${action} action triggered with value: ${value}`);
        // Your action logic for each button (e.g., sending value to server or performing a calculation)
    } else {
        console.log(`${action} input field is empty`);
    }
}

// Handle dropdown action (get the selected option)
function dropdownAction() {
    const selectedOption = document.getElementById('optionsDropdown').value;
    console.log(`Dropdown selected: ${selectedOption}`);
    // Add your logic for what to do with the selected option
}

// Handle video file submission (when the Submit button is clicked)
function submitForm() {
    const videoFile = document.getElementById('videoUpload').files[0];
    
    if (videoFile) {
        console.log(`Video file selected: ${videoFile.name}`);
        // Add your logic to upload the video or process the video file here
    } else {
        console.log('No video file selected');
    }
}

// Handle file upload and display video in the mini player
document.getElementById('videoUpload').addEventListener('change', function(event) {
    ...
    if (file) {
        ...
        // Set the source of the video player to the uploaded file's URL
